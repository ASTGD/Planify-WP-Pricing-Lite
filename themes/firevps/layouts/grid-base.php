<?php
/**
 * FireVPS grid layout (base/fallback).
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }
// Allow preset partials to adjust rendering without duplicating the full layout.
$grid_options    = isset( $grid_options ) && is_array( $grid_options ) ? $grid_options : [];
$spec_icon_mode  = isset( $grid_options['spec_icons'] ) ? $grid_options['spec_icons'] : 'icons'; // icons|bullets
$cta_mode        = isset( $grid_options['cta_mode'] ) ? $grid_options['cta_mode'] : 'inline_and_bottom'; // inline_and_bottom|bottom_only
$trust_location  = isset( $grid_options['trust_location'] ) ? $grid_options['trust_location'] : 'inline'; // inline|bottom
if ( $style_block ) {
				...
				<article class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $plan_classes ) ) ); ?>"
					...
					<div class="fvps-card__top">
                        ...
                        <div class="fvps-card__cta-inline"<?php echo $cta_hidden ? ' hidden' : ''; ?>>
                            ...
                            <?php if ( $trust_trio_enabled && $plan_trust_items && 'inline' === $trust_location ) : ?>
					...
					<?php if ( $ordered_specs ) : ?>
						<ul class="pwpl-plan__specs fvps-card__specs<?php echo ( 'bullets' === $spec_icon_mode ) ? ' fvps-card__specs--bullets' : ''; ?>">
							<?php foreach ( $ordered_specs as $spec ) :
								$slug = $spec['slug'] ?? '';
								?>
								<li class="fvps-spec fvps-spec--<?php echo esc_attr( $slug ?: 'generic' ); ?>">
									<?php if ( 'bullets' === $spec_icon_mode ) : ?>
										<span class="fvps-spec__bullet" aria-hidden="true">•</span>
									<?php else : ?>
										<?php
										$icon_id    = '';
										if ( ! empty( $spec['icon'] ) && is_string( $spec['icon'] ) ) {
					...
					<div class="pwpl-plan__cta fvps-card__cta" data-pwpl-cta>
                        ...
                        <?php if ( $trust_trio_enabled && $plan_trust_items && 'bottom' === $trust_location ) : ?>
                            <ul class="fvps-cta-trust<?php echo count( $plan_trust_items ) > 3 ? ' fvps-cta-trust--stack' : ''; ?>" role="list">
                                <?php foreach ( $plan_trust_items as $item ) : ?>
                                    <li role="listitem"><?php echo esc_html( (string) $item ); ?></li>
                                <?php endforeach; ?>
