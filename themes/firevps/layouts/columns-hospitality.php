<?php
/**
 * FireVPS columns layout: Hospitality preset.
 *
 * Uses per-plan hero images (demo URLs or uploaded hero) plus hospitality body/footer styling.
 */
...
?>>
	...
	<div class="fvps-columns-grid">
		<?php foreach ( $plans as $plan_index => $plan ) :
			...
			if ( ! $hero_image_id && ! $hero_image_url && defined( 'PWPL_URL' ) ) {
			...
			$card_body_classes = [ 'fvps-card__body', 'fvps-card__body--hospitality' ];
			$card_footer_classes = [ 'fvps-card__footer', 'fvps-card__footer--hospitality' ];
			?>
