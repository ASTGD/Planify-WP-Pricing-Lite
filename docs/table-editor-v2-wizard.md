...
- Five wizard presets (SaaS 3 Column, Startup Pricing Grid, Feature Comparison Table, Service Plans, App Pricing) map to distinct FireVPS systems (colors, card style, CTA treatment, specs styling) and will use the appropriate layout partial.
- Service Plans is the single Services preset; it now ships with four tiers (Free, Starter, Pro, Premium) and a unified ticked feature list (no per-row icons). Legacy Service Columns remains for existing tables but is hidden from the wizard.
...
- Comparison layout headers are flattened: compact plan title/price with inline CTAs in the header row (no card footers), above a clean spec matrix.
